...
def sampling_benchmark(cfg, env, test_input, act_model):
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    ...
    best_tour = pred_tours[index_lmin]
    best_distance = l_batch[index_lmin].item()
...
def active_search_benchmark(cfg, env, test_input,act_model,act_optim, log_path = None):
	'''
	active search updates model parameters even during inference on a single input
	test input:(city_t,xy)
	'''
