...
def active_search(cfg, env, test_input, log_path=None):
    '''
    Active search updates model parameters even during inference
    on a single input.
    test_input : (city_t, 2)
    '''
    ...
    # ======================================================
    # ===== ADDED =====
    # Initialize the best tour with the first random tour.
    # This prevents UnboundLocalError if Active Search
    # never finds a better solution.
    # ======================================================
    best_tour = random_tours[0].clone()
    ...
    for i in tqdm(range(cfg.steps)):
        ...
        pred_shuffle_tours, neg_log = act_model(
            shuffle_inputs,
            device
        )
        ...
        if l_batch[index_lmin] < l_min:
            ...
            # ======================================================
            # ===== MODIFIED =====
            # Save the improved best tour.
            # ======================================================
            best_tour = pred_tours[index_lmin].clone()

            print(
                'update best tour, min l(%1.3f -> %1.3f)'
        ...
        baseline = (
            baseline * cfg.alpha
            + (1 - cfg.alpha) * torch.mean(l_batch, dim=0)
        )
        ...
        if cfg.islogger:
            ...
            if i % cfg.log_step == 0:
                ...
                if log_path is None:
                    ...
                    log_path = (
                        cfg.log_dir
                        + 'active_search_%s.csv' % date
                    )

                    with open(log_path, 'w') as f:
                        f.write(
                            'step,actic loss,minimum distance\n'
                        )
